        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          <section className="mt-16 sm:mt-20">
            ...
            <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-8">
              {/* Visi Card */}
              <div className="flex flex-col rounded-xl bg-panel p-7 ring-gold-frame sm:p-10">
                <div className="flex items-center gap-3">
                  <span className="flex h-8 w-8 items-center justify-center rounded-full bg-gold/10 font-display text-sm font-bold text-gold">
                    01
                  </span>
                  <h3 className="font-display text-xl font-semibold uppercase tracking-wide text-gold sm:text-2xl">
                    Visi
                  </h3>
                </div>
              ...
              <div className="flex flex-col rounded-xl bg-panel p-7 ring-gold-frame sm:p-10">
                ...
                <ol className="mt-6 space-y-3.5">
                  {visiMisi.misi.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-sm text-cream/85">
                      <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border border-gold/40 text-xs font-semibold text-gold-soft">
                        {idx + 1}
                      </span>
          ...
          <section className="mt-20 border-t border-hairline pt-16 sm:mt-24 sm:pt-20">
            ...
            <div className="mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {tim.members.map((member) => (
                <div
                  key={member.id}
                  className="group flex flex-col items-center rounded-xl bg-panel p-6 text-center transition-all hover:bg-panel-2 ring-gold-frame"
                >
                  <div className="relative mb-5 h-28 w-28 overflow-hidden rounded-full border-2 border-gold/40 shadow-inner">
                    <ImageSlot
                      src={member.photo}
                      alt={member.name}
                  ...
                  <h3 className="font-display text-base font-semibold text-gold transition-colors group-hover:text-gold-soft">
                    {member.name}
                  </h3>
          ...
          <section className="mt-20 border-t border-hairline pt-16 sm:mt-24 sm:pt-20">
            ...
            <div className="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-4 sm:gap-6">
              {kerjasamaLogos.map((item) => (
                <div
                  key={item.id}
                  className="flex h-28 flex-col items-center justify-center rounded-xl bg-panel p-4 ring-gold-frame transition-all hover:bg-panel-2"
                >
                  <ImageSlot
                    src={item.logo}
                    alt={item.name}
                  ...
                  />
                  <span className="mt-2 text-center text-[11px] font-medium text-muted">
                    {item.name}
                  </span>
                </div>
              ))}
                ...
                <div className="flex flex-col justify-between rounded-lg border border-cream-ink/15 bg-cream-2/80 p-5 lg:col-span-6">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-cream-ink/70">
                      Universitas Negeri Surabaya, Kampus Lidah Wetan / Ketintang
                    </p>
                  </div>
                  ...
                  <div className="relative mt-4 h-64 w-full overflow-hidden rounded-md border border-cream-ink/20 bg-cream-ink/5">
                    <iframe
                      title="Lokasi PUI Seni Budaya Majapahitan"
                      src="https://maps.google.com/maps?q=Universitas+Negeri+Surabaya&t=&z=14&ie=UTF8&iwloc=&output=embed"
